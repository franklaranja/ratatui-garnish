...
use ratatui_garnish::{
    GarnishedWidget, garnishes,
    shadow::HalfShadow,
    title::{Above, Title},
};

use palette::{BLUE600, BLUE900, GREEN600, ORANGE50, ORANGE100, ORANGE400, ORANGE500, ORANGE700};
...
fn draw(frame: &mut Frame) {
    ...
    // render title & help
    frame.render_widget(title("ratatui-garnish  Shadow Demo"), title_area);
    frame.render_widget(help(), help_area);

    let quote = Text::from(vec![
        Line::raw(
            "Rust offers control over low-level details in combination with high-level ergonomics, eliminating the hassle traditionally associated with low-level languages.",
        ),
        Line::raw(""),
        Line::styled(
            "Steve Klabnik & Carol Nichols",
            Style::default().fg(GREEN600).add_modifier(Modifier::ITALIC),
        ),
    ]);
